...
@section('page-script')
    ...
    <script>
        $(document).ready(function() {
            ...
            let state_id = getUrlParameter('state_id');
            window.deleteOptions('Branch');
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('branch.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Branches') }}</h5>
                ...
                <div class="col-md-4">
                    @can('create_branch')
                        <a href="{{ route('branch.create') }}" class="btn btn-primary mb-3"
                            id="newUserButton">{{ _t('New Branch') }}</a>
                    @endcan
                </div>
            </div>
        </div>
