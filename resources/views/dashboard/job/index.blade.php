    ...
    <script>
        $(document).ready(function() {
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('job.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Jobs') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                ...
                <div class="col-md-4">
                    @can('create_job')
                        <!-- <a href="{{ route('job.create') }}" class="btn btn-primary mb-3" id="newUserButton">{{ _t('New job') }}</a> -->
                    @endcan
                </div>
            </div>
        </div>
