        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="post" action="{{route('job.update', $job->id)}}" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label" for="basic-icon-default-fullname">{{_t('Title')}} *</label>
                        <div class="col-sm-10">
                            <div class="input-group input-group-merge">
                                <span id="basic-icon-default-fullname2" class="input-group-text"><i class="ti ti-user"></i></span>
                                <input type="text" name="title" value="{{$job->title}}" disabled required class="form-control" id="basic-icon-default-fulltitle" placeholder="{{_t('title')}}" aria-label="{{_t('title')}}" aria-describedby="basic-icon-default-fulltitle2" required />
                            </div>
                            @error('title')
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group input-group-merge">
                                <select name="section" class="form-select select2" disabled>
                                    @foreach(App\Enums\SectionEnum::cases() as $section)
                                    <option value="{{$section->value}}" {{$job->section->value == $section->value ? 'selected' : ''}}>{{$section->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('section')
                    ...
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label" for="basic-icon-default-email">{{_t('Description')}} *</label>
                        <div class="col-sm-10">
                            <div class="input-group input-group-merge">
                                <span class="input-group-text"><i class="ti ti-home"></i></span>
                                <textarea name="description" id="basic-icon-default-email" class="form-control" aria-label="john.doe" aria-describedby="basic-icon-default-email2" required> {{$job->description}}</textarea>
                            </div>
                            @error('description')
                            <span class="text-danger">{{$message }}</span>
                            @enderror
