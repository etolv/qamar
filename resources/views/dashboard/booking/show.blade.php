...
@section('page-script')
    <script>
        $(document).ready(function() {
            // map
            @if ($booking->addressModel)
                var lng = "{{ $booking->addressModel->lng }}";
                var lat = "{{ $booking->addressModel->lat }}";
            @else
                lng = "{{ $booking->lng }}";
                lat = "{{ $booking->lat }}";
            @endif
            ...
            L.marker([lat, lng]).addTo(map)
                .bindPopup("{{ _t('Booking customer location') }}")
                .openPopup();
            // end map
            var initialSelectedStatus = $('#status').val();
            var initialSelectedDelivery = $('#delivery').val();
            var allowChangeEvent = true;
            $('#status').on('change', function() {
                if (allowChangeEvent) {
                    var selectedStatus = $(this).val();
                    sendApiRequest(selectedStatus, 'ORDER');
                }
            });
            $('#delivery').on('change', function() {
                if (allowChangeEvent) {
                    var selectedDelivery = $(this).val();
                    sendApiRequest(selectedDelivery, 'ORDER_DELIVERY');
                }
            });

            function sendApiRequest(selectedValue, type) {
                ...
                console.log(type);
                $.ajax({
                    url: route('order.change_status'), // Replace with your actual API endpoint
                    method: 'POST',
                    data: formData,
                    success: function(response) {
                        ...
                        toastr.success("done");
                        if (type === 'ORDER') {
                            initialSelectedStatus = selectedValue;
                        } else if (type === 'ORDER_DELIVERY') {
                            initialSelectedDelivery = selectedValue;
                        }
                    },
                    error: function(error) {
                        ...
                        if (type === 'ORDER') {
                            $('#status').val(initialSelectedStatus).trigger('change');
                        } else if (type === 'ORDER_DELIVERY') {
                            $('#delivery').val(initialSelectedDelivery).trigger('change');
                        }
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
            ...
            <div class="d-flex flex-column justify-content-center gap-2 gap-sm-0">
                <h5 class="mb-1 mt-3 d-flex flex-wrap gap-2 align-items-end">{{ _t('Booking') }} #{{ $booking->id }}</h5>
                <h5 class="mb-1 mt-3 d-flex flex-wrap gap-2 align-items-end">
                    @php
                        $color = 'info';
                        if (in_array($booking->status->name, ['CANCELLED', 'REJECTED'])) {
                            $color = 'danger';
                        } elseif (in_array($booking->status->name, ['COMPLETED'])) {
                            $color = 'success';
                        }
                        $payment_color = 'info';
                        if ($booking->payment_status->name == 'PAID') {
                            $payment_color = 'success';
                        } elseif ($booking->payment_status->name == 'UNPAID') {
                            $payment_color = 'danger';
                        }
                    @endphp

                    <span>{{ _t('Status') }}:</span> <span
                        class="badge bg-label-{{ $color }}">{{ $booking->status?->name }}</span>
                    <span>{{ _t('Payment status') }}:</span> <span
                        class="badge bg-label-{{ $payment_color }}">{{ $booking->payment_status?->name }}</span>
                </h5>
                <p class="text-body">{{ $booking->date }}</p>
            </div>
            ...
            <div class="col-12 col-lg-8">
                ...
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div class="col">
                            <h5 class="card-title m-0">{{ _t('Booking details') }}</h5>
                        </div>
                        <!-- <div class="col">
                                <div class="mb-1 mt-1">
                                    <div class="col-sm-10">
    ...
    @endforeach
                        ...
                        <!-- @foreach ([] as $status)
    -->
                        <!-- <option value="{{ $status->value }}" {{ $booking->delivery_status == $status ? 'selected' : '' }}>{{ $status->name }}</option> -->
                        <!--
    @endforeach -->
                        <!-- </select> -->
                        <!-- </div> -->
                    ...
                    <div class="card-datatable table-responsive">
                        <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                            <table class="datatables-order-details table border-top dataTable no-footer dtr-column"
                                id="DataTables_Table_0" style="width: 659px;">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="w-25 sorting_disabled" rowspan="1" colspan="1" style="width: 85px;"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($booking->products as $product)
                                        <tr class="odd">
                                                ...
                                                <div class="d-flex justify-content-start align-items-center text-nowrap">
                                                    <div class="avatar-wrapper">
                                                        <div class="avatar me-2"><img
                                                                src="{{ $product->product->getFirstMedia('image') ? $product->product->getFirstMediaUrl('image') : asset('assets/img/illustrations/default.png') }}"
                                            ...
                                            <td><span>{{ 'SAR ' . $product->price }}</span></td>
                                            <td><span class="text-body">{{ 1 }}</span></td>
                                            <td>
                                                <h6 class="mb-0 text-nowrap">{{ 'SAR ' . $product->price * 1 }}</h6>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            ...
                            <table class="datatables-order-details table border-top dataTable no-footer dtr-column"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="w-50 sorting_disabled" rowspan="1" colspan="1"
                                            style="width: 239px;" aria-label="products">{{ _t('Employee') }}</th>
                                        <th class="w-25 sorting_disabled" rowspan="1" colspan="1" style="width: 94px;"
                                            aria-label="price">{{ _t('price') }}</th>
                                        <th class="w-25 sorting_disabled" rowspan="1" colspan="1"
                                            style="width: 85px;" aria-label="qty">{{ _t('qty') }}</th>
                                        <th class="sorting_disabled" rowspan="1" colspan="1" style="width: 53px;"
                                        ...
                                        <tr class="odd">
                                            ...
                                            <td class="sorting_1">
                                                <div class="d-flex justify-content-start align-items-center text-nowrap">
                                                    <div class="avatar-wrapper">
                                                        <div class="avatar me-2"><img
                                                                src="{{ $bookingService->service->getFirstMedia('image') ? $bookingService->service->getFirstMediaUrl('image') : asset('assets/img/illustrations/default.png') }}"
                                                                alt="" class="rounded-2"></div>
                                                    </div>
                                                    <div class="d-flex flex-column">
                                                        <h6 class="text-body mb-0 text-wrap">
                                                            {{ Illuminate\Support\Str::limit($bookingService->service->name, 30, '...') }}
                                                        </h6>
                                                    </div>
                                                </div>
                                            </td>
                                                ...
                                                @if ($bookingService->employee)
                                                    <div class="d-flex justify-content-start align-items-center mb-4">
                                                        <div class="avatar me-2">
                                                            <img src="{{ $bookingService->employee->user->getFirstMedia('profile') ? $orderService->employee->user->getFirstMediaUrl('profile') : asset('assets/img/illustrations/NoImage.png') }}"
                                                        ...
                                                        <div class="d-flex flex-column">
                                                            <a href="https://demos.pixinvent.com/vuexy-html-laravel-admin-template/demo-1/app/user/view/account"
                                                                ...
                                                                <h6 class="mb-0">
                                                                    {{ $bookingService->employee->user->name }}</h6>
                                                            </a>
                                                            <small
                                                                class="text-muted">{{ $bookingService->employee?->user?->phone }}</small>
                                                        </div>
                                                    </div>
                                                @else
                                                    <span>{{ _t('Not Assigned') }}</span>
                                                @endif
                                            </td>
                                            <td><span>{{ 'SAR ' . $bookingService->price }}</span></td>
                                            <td><span class="text-body">{{ 1 }}</span></td>
                                            <td>
                                                <h6 class="mb-0 text-nowrap">{{ 'SAR ' . $bookingService->price * 1 }}
                                                </h6>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="d-flex justify-content-end align-items-center m-3 mb-2 p-1">
                            <div class="order-calculations">
                                ...
                                <div class="d-flex justify-content-between mb-2">
                                    ...
                                    <h6 class="mb-0">{{ _t('SAR') }} {{ $booking->tax }}</h6>
                                </div>
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="w-px-100 text-heading">{{ _t('Discount') }}:</span>
                                    <h6 class="mb-0">{{ _t('SAR') }} {{ $booking->discount }}</h6>
                                </div>
                ...
                <!-- rates -->
                @if (count($booking->rates))
                    <div class="card mb-4">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <div class="col">
                                <h5 class="card-title m-0">{{ _t('Booking Rates') }}</h5>
                            </div>
                        </div>
                        <div class="card-datatable table-responsive">
                            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                                <table class="datatables-order-details table border-top dataTable no-footer dtr-column"
                                    id="DataTables_Table_0" style="width: 650px;">
                                    <thead>
                                        <tr>
                                            <th class="w-25 sorting_disabled" rowspan="1" colspan="1"
                                                style="width: 90px;" aria-label="{{ _t('Type') }}">
                                                {{ _t('Type') }}</th>
                                            <th class="w-25 sorting_disabled" rowspan="1" colspan="1"
                                                style="width: 94px;" aria-label="{{ _t('Rate') }}">
                                                {{ _t('Rate') }}</th>
                                            <th class="w-50 sorting_disabled" rowspan="1" colspan="1"
                                                style="width: 150px;" aria-label="{{ _t('Description') }}">
                                                {{ _t('Description') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>

                                        @foreach ($booking->rates as $rate)
                                            <tr class="odd">
                                                <td class="  control" tabindex="0" style="display: none;"></td>
                                                <td><span>{{ _t($rate->type->name) }}</span></td>
                                                <td><span class="text-body">{{ $rate->rate }}</span></td>
                                                <td>
                                                    <h6 class="mb-0 text-nowrap">{{ $rate->description }}</h6>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            <div class="d-flex justify-content-end align-items-center m-3 mb-2 p-1">
                                <div class="order-calculations">
                                    <div class="d-flex justify-content-between mb-2">
                                        <span class="w-px-100 text-heading">{{ _t('Rate AVG') }}:</span>
                                        <h6 class="mb-0">{{ $booking->rates->average('rate') }}</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <h5 class="card-title m-0">{{ _t('Booking statuses') }}</h5>
                    </div>
                    <div class="card-body">
                        <ul class="timeline pb-0 mb-0">
                            @if (!in_array($booking->status->value, [5, 6]))
                                <li
                                    class="timeline-item timeline-item-transparent {{ $booking->status->value >= 1 ? 'border-primary' : 'border-transparent pb-0' }}">
                                    <span
                                        class="timeline-point {{ $booking->status->value >= 1 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0' }}"></span>
                                    <div class="timeline-event">
                                        <div class="timeline-header">
                                            <h6 class="mb-0">{{ _t('Pending') }}</h6>
                                            <span
                                                class="text-muted">{{ $booking->status->value >= 1 ? _t('DONE') : _t('PENDING') }}</span>
                                        </div>
                                        <!-- <p class="mt-2">{{ $booking->status->value >= 1 ? _t('DONE') : _t('PENDING') }}</p> -->
                                    </div>
                                </li>
                                <li
                                    class="timeline-item timeline-item-transparent {{ $booking->status->value >= 2 ? 'border-primary' : 'border-transparent pb-0' }}">
                                    <span
                                        class="timeline-point {{ $booking->status->value >= 2 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0' }}"></span>
                                    <div class="timeline-event">
                                        <div class="timeline-header">
                                            <h6 class="mb-0">{{ _t('Accepted') }}</h6>
                                            <span
                                                class="text-muted">{{ $booking->status->value >= 2 ? _t('DONE') : _t('PENDING') }}</span>
                                        </div>
                                        <!-- <p class="mt-2">{{ $booking->status->value >= 2 ? _t('DONE') : _t('PENDING') }}</p> -->
                                    </div>
                                </li>
                                <li
                                    class="timeline-item timeline-item-transparent {{ $booking->status->value >= 3 ? 'border-primary' : 'border-transparent pb-0' }}">
                                    <span
                                        class="timeline-point {{ $booking->status->value >= 3 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0' }}"></span>
                                    <div class="timeline-event">
                                        <div class="timeline-header">
                                            <h6 class="mb-0">{{ _t('Confirmed') }}</h6>
                                            <span
                                                class="text-muted">{{ $booking->status->value >= 3 ? _t('DONE') : _t('PENDING') }}</span>
                                        </div>
                                        <!-- <p class="mt-2">{{ $booking->status->value >= 3 ? _t('DONE') : _t('PENDING') }}</p> -->
                                    </div>
                                </li>
                                <li
                                    class="timeline-item timeline-item-transparent {{ $booking->status->value >= 4 ? 'border-primary' : 'border-transparent pb-0' }}">
                                    <span
                                        class="timeline-point {{ $booking->status->value >= 4 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0' }}"></span>
                                    <div class="timeline-event">
                                        <div class="timeline-header">
                                            <h6 class="mb-0">{{ _t('Started') }}</h6>
                                            <span
                                                class="text-muted">{{ $booking->status->value >= 4 ? _t('DONE') : _t('PENDING') }}</span>
                                        </div>
                                        <!-- <p class="mt-2">{{ $booking->status->value >= 4 ? _t('DONE') : _t('PENDING') }}</p> -->
                                    </div>
                                </li>
                                <li
                                    class="timeline-item timeline-item-transparent {{ $booking->status->value == 7 ? 'border-primary' : 'border-transparent pb-0' }}">
                                    <span
                                        class="timeline-point {{ $booking->status->value == 7 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0' }}"></span>
                                    <div class="timeline-event">
                                        <div class="timeline-header">
                                            <h6 class="mb-0">{{ _t('Completed') }}</h6>
                                            <span
                                                class="text-muted">{{ $booking->status->value == 7 ? _t('DONE') : _t('PENDING') }}</span>
                                        </div>
                                        <!-- <p class="mt-2">{{ $booking->status->value == 7 ? _t('DONE') : _t('PENDING') }}</p> -->
                                    </div>
                                </li>
                            @else
                                <li class="timeline-item timeline-item-transparent border-danger">
                                    ...
                                    <div class="timeline-event">
                                        <div class="timeline-header">
                                            <h6 class="mb-0">{{ _t($booking->status->name) }}</h6>
            ...
            <div class="col-12 col-lg-4">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-start align-items-center mb-4">
                            <div class="avatar me-2">
                                <img src="{{ $booking->customer->user->getFirstMedia('profile') ? $booking->customer->user->getFirstMediaUrl('profile') : asset('assets/img/illustrations/NoImage.png') }}"
                                    alt="Avatar" class="rounded-circle">
                            </div>
                            <div class="d-flex flex-column">
                                <a href="https://demos.pixinvent.com/vuexy-html-laravel-admin-template/demo-1/app/user/view/account"
                                    class="text-body text-nowrap">
                                    <h6 class="mb-0">{{ $booking->customer?->user?->name ?? '-' }}</h6>
                                </a>
                                <small class="text-muted">{{ _t('Customer ID') }}: #{{ $booking->customer_id }}</small>
                            </div>
                        </div>
                        <div class="d-flex justify-content-start align-items-center mb-4">
                            <span
                            ...
                            <h6 class="text-body text-nowrap mb-0">{{ $booking->customer?->orders?->count() ?? 0 }}
                                {{ _t('Orders') }}</h6>
                        </div>
                        <div class="d-flex justify-content-start align-items-center mb-4">
                            <span
                            ...
                            <h6 class="text-body text-nowrap mb-0">{{ $booking->customer?->bookings?->count() ?? 0 }}
                                {{ _t('Bookings') }}</h6>
                        </div>
                        <div class="d-flex justify-content-between">
                        ...
                        <p class=" mb-1">{{ _t('City') }}: {{ $booking->customer?->city?->name ?? '-' }}</p>
                        <p class=" mb-0">{{ _t('Mobile') }}: {{ $booking->customer?->user?->phone ?? '-' }}</p>
                    </div>
                </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-header d-flex justify-content-between">
                        <h6 class="card-title m-0">{{ _t('Customer address') }}</h6>
                    ...
                    @if ($booking->addressModel)
                        <div class="card-body">
                            ...
                            <p class="mb-0">{{ $booking->street }}</p>
                            <p class="mb-0">{{ $booking->municipal?->name }}</p>
                        </div>
                    @else
                        <div class="card-body">
                            <p class="mb-0">{{ $booking->address }}</p>
                ...
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between">
                        <h6 class="card-title m-0">{{ _t('Map') }}</h6>
                        <!-- <h6 class="m-0"><a href=" javascript:void(0)" data-bs-toggle="modal" data-bs-target="#addNewAddress">Edit</a></h6> -->
                    </div>
                    <div class="card-body">
                        ...
                        <form id="editUserForm" class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework"
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <div class="position-relative"><select id="modalEditUserStatus"
                                            name="modalEditUserStatus"
                                            class="select2 form-select select2-hidden-accessible"
                                            aria-label="Default select example" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <div class="position-relative"><select id="modalEditUserLanguage"
                                            ...
                                            dir="ltr" data-select2-id="24" style="width: auto;"><span
                                                class="selection"><span
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="position-relative">
                                    <div class="position-relative"><select id="modalEditUserCountry"
                                            ...
                                            dir="ltr" data-select2-id="52" style="width: auto;"><span
                                                class="selection"><span
                                                    class="select2-selection select2-selection--single" role="combobox"
                                                    aria-haspopup="true" aria-expanded="false" tabindex="0"
                                                    aria-disabled="false"
                            ...
                            <div class="col-12">
                                ...
                                <div class="position-relative">
                                    <div class="position-relative"><select id="modalAddressCountry"
                                            ...
                                            dir="ltr" data-select2-id="79" style="width: auto;"><span
                                                class="selection"><span
                                                    class="select2-selection select2-selection--single" role="combobox"
                                                    aria-haspopup="true" aria-expanded="false" tabindex="0"
                                                    aria-disabled="false"
