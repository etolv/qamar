...
@section('page-script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var date = "{{ $date ?? null }}";
            var booking_status = "{{ $status ?? null }}";
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('booking.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
            ...
            if (is_mobile == 1) {
                $('.page-type').text("({{ _t('Application Bookings') }})")
            }
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Bookings') }} <sapn class="text-muted page-type">
                    </span></h5>
                ...
                <div class="col-md-4">
                    @can('create_booking')
                        <a href="{{ route('booking.create') }}" class="btn btn-primary mb-3"
                            id="newUserButton">{{ _t('New Booking') }}</a>
                    @endcan
                </div>
            </div>
        </div>
