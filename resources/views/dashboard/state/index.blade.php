    ...
    <script>
        $(document).ready(function() {
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('state.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            return (
                                ...
                                @can('update_state')
                                    '<a href="' + route('state.edit', data.id) +
                                ...
                                '<a href="' + route('branch.index', {
                                    'state_id': data.id
                                }) + '" class="dropdown-item">{{ _t('Branches') }}</a>' +
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('States') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                ...
                <div class="col-md-4">
                    @can('create_state')
                        <a href="{{ route('state.create') }}" class="btn btn-primary mb-3"
                            id="newUserButton">{{ _t('New State') }}</a>
                    @endcan
                </div>
            </div>
        </div>
            ...
            <table class="datatables-users-test table dt-responsive">
                <thead class="border-top">
                    ...
                    <tr>
                        <th>#</th>
                        <th>{{ _t('Name') }}</th>
                        <th>{{ _t('Cities count') }}</th>
                        <th>{{ _t('Branches count') }}</th>
                        <th>{{ _t('Status') }}</th>
                        <th>{{ _t('Actions') }}</th>
                    </tr>
                </thead>
