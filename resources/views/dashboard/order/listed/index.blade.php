    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('listed-order.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var name = full.order.customer.user.name,
                                image = full.customer_image ||
                                "{{ asset('assets/img/illustrations/NoImage.png') }}",
                                number = full.order.customer.user.phone,
                                baseUrl = route('customer.edit', full.order.customer.id);
                            var row_output =
                    ...
                    //   render: function(data, type, full, meta) {
                    //     if (full.branch) {
                    //       var name = full.order.branch.name,
                    //         number = full.order.branch.city.name,
                    //         baseUrl = route('branch.edit', full.order.branch_id);
                    //       var row_output =
                    //         '<div class="d-flex justify-content-start align-items-center">' +
                    //         '<div class="d-flex flex-column">' +
                    //         '<a href="' +
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var actions =
                                ...
                                '<a href="' + route('notification.user.create', data.order.customer
                                    .user_id) +
                                '" class="dropdown-item"><i class="fa-regular fa-comment"></i> {{ _t('Notification') }}</a>' +
                                '</div>';
                            return (
                                '<div class="d-flex align-items-center">' +
                                '<a href="' + route("order-service-session.show", data.id) +
                                '" title="' + "{{ _t('Sessions') }}" +
                                '" class="text-body"><i class="ti ti-eye ti-sm me-2"></i></a>' +
                                '<a target="_blank" href="https://wa.me/' + data.order.customer
                                .user.phone + '" title="' + "{{ _t('Whatsapp') }}" +
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Listed Orders') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                <div class="col-md-4 user_role"></div>
                <div class="col-md-4 user_plan"></div>
                <div class="col-md-4 user_status"></div>
                <div class="col-md-4">
                    <!-- <a href="{{ route('order.create') }}" class="btn btn-primary mb-3" id="newUserButton">{{ _t('New Order') }}</a> -->
                </div>
            </div>
        </div>
