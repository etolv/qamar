...
@section('title', 'Order Service Postpone')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('order-service-postpone.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var name = full.order_service?.order.customer.user.name,
                                image = full.customer_image ||
                                "{{ asset('assets/img/illustrations/NoImage.png') }}",
                                number = full.order_service?.order.customer.user.phone,
                                baseUrl = route('customer.edit', full.order_service?.order.customer
                                    .id);
                            var row_output =
                    ...
                    },
                    {
                        ...
                        name: 'status',
                        render: function(data, type, full, meta) {
                            var color = data.status == 'PENDING' ? 'warning' : 'success';
                            return (
                    ...
                    {
                        data: 'order_service.service.name',
                        name: 'order_service.service.name'
                    },
                    {
                        orderable: true,
                        searchable: false,
                        data: 'order_service.price',
                        name: 'order_service.price'
                    },
                    {
                        data: 'due_date',
                        name: 'due_date'
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var actions =
                                ...
                                '<a href="' + route('order.show', data.order_service.order_id) +
                                '" class="dropdown-item"><i class="fa-regular fa-eye"></i> {{ _t('View Order') }}</a>' +
                                '<a href="' + route('notification.user.create', data.order_service
                                    .order.customer.user_id) +
                                '" class="dropdown-item"><i class="fa-regular fa-comment"></i> {{ _t('Notification') }}</a>';
                            if (data.status == 'PENDING') {
                                actions +=
                                    `<a href="javascript:;" data-id="${data.id}" data-date="${data.due_date}"
                                    data-bs-toggle="modal" data-bs-target="#editPostponeModal"
                                     class="dropdown-item"><i class="fa-regular fa-pen-to-square"></i> {{ _t('Edit') }}</a>`;
                                actions += '<a href="' + route('order-service-postpone.return', data
                                        .id) +
                                    '" class="dropdown-item"><i class="fa-solid fa-rotate-left"></i> {{ _t('Return') }}</a>';
                                actions += '<a href="' + route('order-service-postpone.complete',
                                        data.id) +
                            ...
                            return (
                                ...
                                '<a target="_blank" href="https://wa.me/' + data.order_service
                                .order.customer.user.phone + '" title="' +
                                "{{ _t('Whatsapp') }}" +
                                '" class="text-body"><i class="fa-brands fa-whatsapp"></i></a>' +
            ...
            $('#editPostponeModal').on('show.bs.modal', function(event) {
                var button = $(event.relatedTarget);
                var id = button.data('id');
                var date = button.data('date');
                var url = route('order-service-postpone.update', id);
                var modal = $(this);
                modal.find('#form-edit').attr('action', url);
                modal.find('#due_date').val(date);
            });
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Order Service Postpone') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                <div class="col-md-4 user_role"></div>
                <div class="col-md-4 user_plan"></div>
                <div class="col-md-4 user_status"></div>
                <div class="col-md-4">
                    {{-- <a href="{{ route('order.create') }}" class="btn btn-primary mb-3"
                        id="newUserButton">{{ _t('New Order') }}</a> --}}
                </div>
            </div>
        </div>
    ...
    <!-- Modal -->
    <div class="modal-onboarding modal fade animate__animated" id="editPostponeModal" tabindex="-1" aria-hidden="true">
