...
@section('page-script')
    <script>
        $(document).ready(function() {
            ...
            var bookingId = getUrlParameter('booking_id');
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('rate.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var name = `${full.model_type} #${full.model_id}`,
                                baseUrl = full.action;
                            var row_output =
                    ...
                    },
                    {
                        ...
                        searchable: false,
                        render: function(data, type, full, meta) {
                            var name = full.model.customer.user.name,
                                image = full.model.customer_image ||
                                "{{ asset('assets/img/illustrations/NoImage.png') }}",
                                number = full.model.customer.user.phone,
                                baseUrl = route('customer.edit', full.model.customer.id);
                            var row_output =
                                '<div class="d-flex justify-content-start align-items-center">' +
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Rates') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                <div class="col-md-4 user_role"></div>
                <div class="col-md-4 user_plan"></div>
                <div class="col-md-4 user_status"></div>
                <div class="col-md-4">
                    <!-- <a href="{{ route('rate.create') }}" class="btn btn-primary mb-3" id="newProductButton">{{ _t('New rate') }}</a> -->
                </div>
            </div>
        </div>
