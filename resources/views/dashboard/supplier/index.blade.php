    ...
    <script>
        $(document).ready(function() {
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('supplier.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
              ...
              <span class="switch-toggle-slider">
                  ...
                  <i class="ti ti-x"></i>
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            let suppliersAction = '';
                            if (!full.supplier_id) {
                                suppliersAction = '<a href="' + route('supplier.index', {
                                    supplier_id: data.id
                                }) + '" class="dropdown-item">{{ _t('Representatives') }}</a>';
                            }
                            return (
                                ...
                                '<a href="' + route('bill.debt', {
                                    supplier_id: data.id
                                }) + '" class="dropdown-item">{{ _t('Debts') }}</a>' +
                                '</div>' +
                                '</div>'
                            );
                        }
                ...
                buttons: [{
                    ...
                    init: function(api, node, config) {
                        console.log(api.columns().header().toArray());
                        console.log(api.columns().data().toArray());
                        if (!supplierId) {
                            api.column(2).visible(false); // name
                            api.column(3).visible(false); // email
                            api.column(4).visible(false); // phone
                        } else {
                            api.column(8).visible(false); // tax number
                            api.column(9).visible(false); // address
                        }
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ $supplier ? _t('Representatives') : _t('Suppliers') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                ...
                <div class="col-md-4">
                    @if (!$supplier || !$supplier->supplier_id)
                        <a href="{{ route('supplier.create', ['supplier_id' => request()->supplier_id]) }}"
                            class="btn btn-primary mb-3"
                            id="newUserButton">{{ $supplier ? _t('New Representative') : _t('New Supplier') }}</a>
                    @endif
                </div>
            </div>
        </div>
