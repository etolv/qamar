                ...
                <div class="card-body">
                    <form method="post" action="{{ route('supplier.update', $supplier->id) }}" enctype="multipart/form-data">
                        ...
                        @if ($supplier->supplier)
                            <div class="main-supplier-div">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="input-group input-group-merge">
                                            ...
                                            <input type="text" name="name"
                                                ...
                                                id="basic-icon-default-fullname" placeholder="{{ _t('Name') }}" />
                                        </div>
                                        @error('name')
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        @include('components.phone_with_code', ['user' => $supplier])
                                        @error('phone')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                        ...
                                        <div class="input-group input-group-merge">
                                            ...
                                            <div class="col-sm-10">
                                                <select id="supplier_id" name="supplier_id"
                                                    class="supplier_select2 form-select" data-allow-clear="false">
                                                    @if ($supplier->supplier)
                                        ...
                                        <select id="type" name="type" class="type_select2 form-select" required>
                                            @foreach ($types as $type)
                                                <option value="{{ $type->value }}"
                                                    {{ $supplier->type == $type ? 'selected' : '' }}>{{ _t($type->name) }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <div class="col-sm-10">
                                <div class="input-group input-group-merge">
                                    ...
                                    <textarea name="address" value="{{ old('address') }}" id="basic-icon-default-email" class="form-control"
                                        aria-label="john.doe" aria-describedby="basic-icon-default-email2"> {{ old('address') ?? $supplier->address }}</textarea>
                                </div>
                                @error('address')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                        ...
                        @if ($supplier->supplier_id == null)
                            <div class="row mb-3 company-div">
                                <label class="col-sm-2 col-form-label"
                                    for="basic-icon-default-fullname">{{ _t('Company') }} *</label>
                                <div class="col-sm-10">
                                    <div class="input-group input-group-merge">
                                        <span id="basic-icon-default-fullname2" class="input-group-text"><i
                                                class="fa-solid fa-building"></i></span>
                                        <input type="text" name="company"
                                            value="{{ old('company') ?? $supplier->company }}" required
                                            class="form-control" id="basic-icon-default-fullcompany"
                                            placeholder="{{ _t('company') }}" aria-label="{{ _t('company') }}"
                                            aria-describedby="basic-icon-default-fullcompany2" required />
                                    </div>
                                    @error('company')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                        ...
                        @endif
                        <div class="row mb-3 tax-div {{ $supplier->type->value == 1 ? '' : 'd-none' }}">
                            <label class="col-sm-2 col-form-label"
                            ...
                            <div class="col-sm-10">
                                <div class="input-group input-group-merge">
                                    ...
                                    <input type="text" name="tax_number"
                                        value="{{ old('tax_number') ?? $supplier->tax_number }}" class="form-control"
                                        id="tax_number" placeholder="{{ _t('Tax number') }}" />
                                </div>
                                @error('tax_number')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="row mb-3 link-div {{ $supplier->type->value == 2 ? '' : 'd-none' }}">
                            <label class="col-sm-2 col-form-label"
                            ...
                            <div class="col-sm-10">
                                <div class="input-group input-group-merge">
                                    ...
                                    <input type="url" name="link" value="{{ old('link') ?? $supplier->link }}"
                            ...
                            <div class="col-sm-10">
                                <div class="input-group input-group-merge">
                                    ...
                                    <input type="text" name="bank_number"
                                        value="{{ old('bank number') ?? $supplier->bank_number }}" class="form-control"
                                        id="basic-icon-default-fullbank_number" placeholder="{{ _t('bank_number') }}"
                                        aria-label="{{ _t('bank_number') }}"
                            ...
                            <div class="col-sm-10">
                                ...
                                @if ($supplier->getFirstMedia('profile')?->getUrl())
                                    <div class="col-lg-6 col-md-6 mb-sm-0">
                                        <img src="{{ $supplier->getFirstMedia('profile')?->getUrl() }}" id="profile-img"
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="card-repeater-items">
                                    @foreach ($supplier->cards as $index => $card)
                                        <div class="card-repeater-item">
                                            ...
                                            <div class="row attribute-row mb-3">
                                                <div class="col-md-3">
                                                    <label class="form-label"
                                                        for="attributes">{{ _t('Name') }}</label>
                                                    <input type="text" class="form-control"
                                                        id="cards_name{{ $index }}"
                                                        placeholder="{{ _t('Name') }}" value="{{ $card->name }}"
                                                        name="cards[{{ $index }}][name]" required>
                                                </div>
                                                <div class="col-md-4">
                                                    <label class="form-label"
                                                        for="attributes">{{ _t('Number') }}</label>
                                                    <input type="text" class="form-control"
                                                        id="cards_number{{ $index }}"
                                                        placeholder="{{ _t('Number') }}" value="{{ $card->number }}"
                                                        name="cards[{{ $index }}][number]" required>
                                                </div>
                                                <div class="col-md-4">
                                                    <label class="form-label"
                                                        for="attributes">{{ _t('IBAN') }}</label>
                                                    <input type="text" class="form-control"
                                                        id="cards_iban{{ $index }}"
                                                        placeholder="{{ _t('IBAN') }}" value="{{ $card->iban }}"
                                                        name="cards[{{ $index }}][iban]" required>
                                                </div>
                                                <div class="col-md-1 d-flex align-items-end">
                                                    <button type="button" class="btn btn-danger card-remove-attribute"
                                                        title="{{ _t('Remove') }}"><i
