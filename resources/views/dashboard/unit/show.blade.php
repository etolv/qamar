...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        ...
        <div class="d-flex flex-column justify-content-center gap-2 gap-sm-0">
            <h5 class="mb-1 mt-3 d-flex flex-wrap gap-2 align-items-end">{{_t('Customer')}} #{{$customer->id}}</h5>
            <h5 class="mb-1 mt-3 d-flex flex-wrap gap-2 align-items-end">
            </h5>
            <p class="text-body">{{_t('Join date:') . ' ' . $customer->created_at}}</p>
        </div>
        <div class="d-flex align-content-center flex-wrap gap-2">
            <!-- <button class="btn btn-label-danger delete-order waves-effect">Delete Customer</button> -->
        </div>
    </div>
        ...
        <div class="col-12 col-lg-8">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div class="col">
                        <h5 class="card-title m-0">{{_t('Wallet')}}</h5>
                    </div>
                    <div class="col">
                        <div class="mb-1 mt-1">
                        </div>
                ...
                <div class="card-datatable table-responsive">
                    <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                        <table class="datatables-order-details table border-top dataTable no-footer dtr-column" id="DataTables_Table_0" style="width: 685px;">
                            <thead>
                                <tr>
                                    <th class="w-20 sorting_disabled" rowspan="1" colspan="1" aria-label="products">{{_t('Transaction')}}</th>
                                    <th class="w-20 sorting_disabled" rowspan="1" colspan="1" aria-label="price">{{_t('Amount')}}</th>
                                    <th class="w-20 sorting_disabled" rowspan="1" colspan="1" aria-label="qty">{{_t('Balance')}}</th>
                                    <th class="w-20 sorting_disabled" rowspan="1" colspan="1" aria-label="qty">{{_t('Currency')}}</th>
                                    <th class="w-20 sorting_disabled" rowspan="1" colspan="1" aria-label="total">{{_t('Date')}}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($wallet->data->transactions as $index => $transaction)
                                <!-- Display store header when a new store is encountered -->
                                <tr>
                                    <td class="store-header">
                                        <h5 class="text-muted">{{$transaction->message}}</h5>
                                    </td>
                                    <td class="store-header">
                                        {{$transaction->amount}}
                                    </td>
                    ...
                    <div class="d-flex justify-content-end align-items-center m-3 mb-2 p-1">
                        <div class="order-calculations">
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <h6 class="mb-0">{{$app_currency}} {{$wallet->data->amount}}</h6>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-start align-items-center mb-4">
                        <div class="avatar me-2">
                            <img src="{{$customer->user->getFirstMediaUrl('profile') ?? asset('assets/img/illustrations/NoImage.png')}}" alt="Avatar" class="rounded-circle">
                        </div>
                        <div class="d-flex flex-column">
                            <a href="https://demos.pixinvent.com/vuexy-html-laravel-admin-template/demo-1/app/user/view/account" class="text-body text-nowrap">
                                <h6 class="mb-0">{{$customer->user->name}}</h6>
                            </a>
                            <small class="text-muted">{{_t('Customer ID')}}: #{{$customer->id}}</small>
                        </div>
                    </div>
                    <div class="d-flex justify-content-start align-items-center mb-4">
                        ...
                        <h6 class="text-body text-nowrap mb-0">{{$customer->orders->count()}} {{_t('Orders')}}</h6>
                    </div>
                    <div class="d-flex justify-content-start align-items-center mb-4">
                        ...
                        <h6 class="text-body text-nowrap mb-0">{{$customer->bookings->count()}} {{_t('Bookings')}}</h6>
                    </div>
                    <div class="d-flex justify-content-between">
                        <h6>{{_t('Contact info')}}</h6>
                        <!-- <h6><a href=" javascript:void(0)" data-bs-toggle="modal" data-bs-target="#editUser">Edit</a></h6> -->
                    </div>
                    <p class=" mb-1">{{_t('Email')}}: {{$customer->user->email}}</p>
                    <p class=" mb-0">{{_t('Mobile')}}: {{$customer->user->phone}}</p>
                </div>
            </div>

            @php
            $address = $customer->user->addresses->first();
            @endphp
            @if($address)
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between">
                    <h6 class="card-title m-0">{{_t('Shipping address')}}</h6>
                    <h6 class="m-0">{{$address->title}}</h6>
                    <!-- <h6 class="m-0"><a href=" javascript:void(0)" data-bs-toggle="modal" data-bs-target="#addNewAddress">Edit</a></h6> -->
                </div>
                <div class="card-body">
                    <p class="mb-0">{{$address->address}}<br>{{$address->street}} <br>{{$address->building}} <br>{{$address->additional}}</p>
                </div>
