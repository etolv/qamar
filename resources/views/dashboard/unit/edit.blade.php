                ...
                <form method="post" action="{{route('unit.update', $unit->id)}}" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" name="name" value="{{$unit->name}}" required class="form-control" id="basic-icon-default-fullname" placeholder="{{_t('Name')}}" aria-label="{{_t('Name')}}" aria-describedby="basic-icon-default-fullname2" required />
                            </div>
                            @error('name')
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <div class="col-sm-10">
                                    <select id="unit_id" name="unit_id" class="unit_select2 form-select" data-allow-clear="true">
                                        <option value="{{$unit->unit_id}}">{{$unit->parent?->name}}</option>
                                    </select>
                                </div>
                            </div>
                            @error('unit_id')
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group input-group-merge">
                                <span id="basic-icon-default-fullname2" class="input-group-text"><i class="ti ti-tag"></i></span>
                                <input type="number" name="factor" value="{{old('factor') ?? $unit->factor}}" class="form-control" id="basic-icon-default-factor" placeholder="{{_t('factor')}}" aria-label="{{_t('factor')}}" aria-describedby="basic-icon-default-factor2" required />
                            </div>
                            @error('factor')
                            <span class="text-danger">{{$message }}</span>
                            @enderror
