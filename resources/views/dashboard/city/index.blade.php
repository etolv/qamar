    ...
    <script>
        $(document).ready(function() {
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('city.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Cities') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                ...
                <div class="col-md-4">
                    @can('create_city')
                        <a href="{{ route('city.create') }}" class="btn btn-primary mb-3"
                            id="newUserButton">{{ _t('New City') }}</a>
                    @endcan
                </div>
            </div>
        </div>
