    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        ...
        <div class="d-flex flex-column justify-content-center gap-2 gap-sm-0">
            ...
            <h5 class="mb-1 mt-3 d-flex flex-wrap gap-2 align-items-end">
                ...
                <span>{{_t('Status')}}:</span> <span class="badge bg-label-{{$color}}">{{$order->status?->name}}</span>
                <span>{{_t('Payment status')}}:</span> <span class="badge bg-label-{{$payment_color}}">{{$order->payment_status?->name}}</span>
            </h5>
            <p class="text-body">{{$order->date}}</p>
        </div>
        <div class="d-flex align-content-center flex-wrap gap-2">
            <p class="text-body"><a target="_blank" class="btn btn-primary" title="{{_t('Download PDF')}}" href="{{route('order.pdf', $order->id)}}"><i class="fa-solid fa-file-pdf"></i></a></p>
            <p class="text-body"><a target="_blank" class="btn btn-warning" title="{{_t('Rate')}}" href="{{route('order.rate', $order->id)}}"><i class="fa-regular fa-star"></i> </a></p>
    ...
    <div class="row">
        <div class="col-12 col-lg-8">
            <div class="card mb-4">
                ...
                <div class="card-datatable table-responsive">
                        ...
                        <table class="datatables-order-details table border-top dataTable no-footer dtr-column" id="DataTables_Table_0">
                            ...
                            <tbody>
                                ...
                                <tr class="odd">
                                    ...
                                    <td class="sorting_1">
                                        <div class="d-flex justify-content-start align-items-center text-nowrap">
                                            <div class="avatar-wrapper">
                                                <div class="avatar me-2"><img src="{{$orderStock->stock->product->getFirstMedia('image') ? $orderStock->stock->product->getFirstMediaUrl('image') : asset('assets/img/illustrations/default.png')}}" alt="" class="rounded-2"></div>
                                            </div>
                                            <div class="d-flex flex-column">
                                                <h6 class="text-body mb-0 text-wrap">{{ Illuminate\Support\Str::limit($orderStock->stock->product->name, 30, '...') }}</h6>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <h6 class="mb-0 text-nowrap">{{ 'SAR ' . ($orderStock->price * $orderStock->quantity) }}</h6>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        ...
                        <table class="datatables-order-details table border-top dataTable no-footer dtr-column" id="DataTables_Table_0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="w-25 sorting_disabled" rowspan="1" colspan="1" style="width: 94px;" aria-label="price">{{_t('Type')}}</th>
                                    <th class="w-40 sorting_disabled" rowspan="1" colspan="1" style="width: 239px;" aria-label="Employee">{{_t('Employee')}}</th>
                                    <th class="w-40 sorting_disabled" rowspan="1" colspan="1" style="width: 239px;" aria-label="Status">{{_t('Status')}}</th>
                                    <th class="w-25 sorting_disabled" rowspan="1" colspan="1" style="width: 94px;" aria-label="price">price</th>
                                    <th class="w-15 sorting_disabled" rowspan="1" colspan="1" style="width: 85px;" aria-label="qty">qty</th>
                                    <th class="w-15 sorting_disabled" rowspan="1" colspan="1" style="width: 53px;" aria-label="total">total</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr class="odd">
                                    ...
                                    <td class="sorting_1">
                                        <div class="d-flex justify-content-start align-items-center text-nowrap">
                                            <div class="avatar-wrapper">
                                                <div class="avatar me-2"><img src="{{$orderService->service->getFirstMedia('image') ? $orderService->service->getFirstMediaUrl('image') : asset('assets/img/illustrations/default.png')}}" alt="" class="rounded-2"></div>
                                            </div>
                                            <div class="d-flex flex-column">
                                                <h6 class="text-body mb-0 text-wrap">{{ Illuminate\Support\Str::limit($orderService->service->name, 30, '...') }}</h6>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <div class="d-flex justify-content-start align-items-center mb-4">
                                            <div class="avatar me-2">
                                                <img src="{{$orderService->employee->user->getFirstMedia('profile') ? $orderService->employee->user->getFirstMediaUrl('profile') :asset('assets/img/illustrations/NoImage.png')   }}" alt="Avatar" class="rounded-circle">
                                            </div>
                                            <div class="d-flex flex-column">
                                                <a href="https://demos.pixinvent.com/vuexy-html-laravel-admin-template/demo-1/app/user/view/account" class="text-body text-nowrap">
                                                    <h6 class="mb-0">{{$orderService->employee->user->name}}</h6>
                                                </a>
                                                <small class="text-muted">{{$orderService->employee?->user?->phone}}</small>
                                            </div>
                                        </div>
                                        @else
                                    ...
                                    <td><span>{{ $orderService->status->name }}</span></td>
                                    <td><span>{{ 'SAR ' . $orderService->price }}</span></td>
                                    <td><span class="text-body">{{ $orderService->quantity }}</span></td>
                                    <td>
                                        <h6 class="mb-0 text-nowrap">{{ 'SAR ' . ($orderService->price * $orderService->quantity) }}</h6>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                    ...
                    @if($order->coupon)
                    <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                        <table class="datatables-order-details table border-top dataTable no-footer dtr-column" id="DataTables_Table_0" style="width: 685px;">
                            <thead>
                                <tr>
                                    <th class="control sorting_disabled dtr-hidden" rowspan="1" colspan="1" style="width: 0px; display: none;" aria-label=""></th>
                                    <th class="w-25 sorting_disabled" rowspan="1" colspan="1" style="width: 239px;" aria-label="products">{{_t('Coupon')}}</th>
                                    <th class="w-25 sorting_disabled" rowspan="1" colspan="1" style="width: 94px;" aria-label="price">{{_t('Code')}}</th>
                                    <th class="w-15 sorting_disabled" rowspan="1" colspan="1" style="width: 85px;" aria-label="qty">{{_t('Type')}}</th>
                                    <th class="w-15 sorting_disabled" rowspan="1" colspan="1" style="width: 53px;" aria-label="total">{{_t('Value')}}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="odd">
                                    ...
                                    <td>
                                        <h6 class="mb-0 text-nowrap">{{ $order->coupon->is_percentage ? _t('Percentage') : _t('Fixed') }}</h6>
                                    </td>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <ul class="timeline pb-0 mb-0">
                        ...
                        <li class="timeline-item timeline-item-transparent {{$order->status->value >= 1 ? 'border-primary' : 'border-transparent pb-0'}}">
                            <span class="timeline-point {{$order->status->value >= 1 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0'}}"></span>
                            <div class="timeline-event">
                                <div class="timeline-header">
                                    <h6 class="mb-0">{{_t('Pending')}}</h6>
                                    <span class="text-muted">{{$order->status->value >= 1 ? _t('DONE') : _t('PENDING')}}</span>
                                </div>
                                <!-- <p class="mt-2">{{$order->status->value >= 1 ? _t('DONE') : _t('PENDING')}}</p> -->
                            </div>
                        </li>
                        <li class="timeline-item timeline-item-transparent {{$order->status->value >= 2 ? 'border-primary' : 'border-transparent pb-0'}}">
                            <span class="timeline-point {{$order->status->value >= 2 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0'}}"></span>
                            <div class="timeline-event">
                                <div class="timeline-header">
                                    <h6 class="mb-0">{{_t('Accepted')}}</h6>
                                    <span class="text-muted">{{$order->status->value >= 2 ? _t('DONE') : _t('PENDING')}}</span>
                                </div>
                                <!-- <p class="mt-2">{{$order->status->value >= 2 ? _t('DONE') : _t('PENDING')}}</p> -->
                            </div>
                        </li>
                        <li class="timeline-item timeline-item-transparent {{$order->status->value >= 3 ? 'border-primary' : 'border-transparent pb-0'}}">
                            <span class="timeline-point {{$order->status->value >= 3 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0'}}"></span>
                            <div class="timeline-event">
                                <div class="timeline-header">
                                    <h6 class="mb-0">{{_t('Confirmed')}}</h6>
                                    <span class="text-muted">{{$order->status->value >= 3 ? _t('DONE') : _t('PENDING')}}</span>
                                </div>
                                <!-- <p class="mt-2">{{$order->status->value >= 3 ? _t('DONE') : _t('PENDING')}}</p> -->
                            </div>
                        </li>
                        <li class="timeline-item timeline-item-transparent {{$order->status->value >= 4 ? 'border-primary' : 'border-transparent pb-0'}}">
                            <span class="timeline-point {{$order->status->value >= 4 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0'}}"></span>
                            <div class="timeline-event">
                                <div class="timeline-header">
                                    <h6 class="mb-0">{{_t('Started')}}</h6>
                                    <span class="text-muted">{{$order->status->value >= 4 ? _t('DONE') : _t('PENDING')}}</span>
                                </div>
                                <!-- <p class="mt-2">{{$order->status->value >= 4 ? _t('DONE') : _t('PENDING')}}</p> -->
                            </div>
                        </li>
                        <li class="timeline-item timeline-item-transparent {{$order->status->value == 7 ? 'border-primary' : 'border-transparent pb-0'}}">
                            <span class="timeline-point {{$order->status->value == 7 ? 'timeline-point-primary' : 'timeline-point-secondary pb-0'}}"></span>
                            <div class="timeline-event">
                                <div class="timeline-header">
                                    <h6 class="mb-0">{{_t('Completed')}}</h6>
                                    <span class="text-muted">{{$order->status->value == 7 ? _t('DONE') : _t('PENDING')}}</span>
                                </div>
        ...
        <div class="col-12 col-lg-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-start align-items-center mb-4 row">
                        ...
                        <div class="col-md-8">
                            @for($i=1;$i<=5;$i++) @if($order_rate->rate >= $i)
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <img src="https://www.qr-genereren.nl/qrcode.png?text={{route('order.show', $order->id)}}&fore…0&backgroundColor=ffffff&moduleSize=16&padding=0&" class="img-thumbnail img-responsive" />
                </div>
            </div>
        </div>
    </div>
                    ...
                    <form id="editUserForm" class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" onsubmit="return false" novalidate="novalidate">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <div class="position-relative">
                                <div class="position-relative"><select id="modalEditUserStatus" name="modalEditUserStatus" class="select2 form-select select2-hidden-accessible" aria-label="Default select example" tabindex="-1" aria-hidden="true" data-select2-id="modalEditUserStatus">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <div class="position-relative">
                                <div class="position-relative"><select id="modalEditUserLanguage" name="modalEditUserLanguage" class="select2 form-select select2-hidden-accessible" multiple="" tabindex="-1" aria-hidden="true" data-select2-id="modalEditUserLanguage">
                                    ...
                                    </select><span class="select2 select2-container select2-container--default" dir="ltr" data-select2-id="24" style="width: auto;"><span class="selection"><span class="select2-selection select2-selection--multiple" role="combobox" aria-haspopup="true" aria-expanded="false" tabindex="-1" aria-disabled="false">
                                                <ul class="select2-selection__rendered">
                                                    <li class="select2-selection__choice" title="English" data-select2-id="26"><span class="select2-selection__choice__remove" role="presentation">×</span>English</li>
