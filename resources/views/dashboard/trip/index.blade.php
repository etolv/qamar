...
@section('page-script')
    <script>
        $(document).ready(function() {
            ...
            let driver_id = getUrlParameter('driver_id');
            let order_id = getUrlParameter('order_id');
            let booking_id = getUrlParameter('booking_id');
            window.deleteOptions('User');
            $('.driver_select2').select2({
                placeholder: '{{ _t('Filter Driver') }}',
                ajax: {
            ...
            var dataTable = $('.datatables-users-test').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('trip.fetch') }}", // Adjust the URL based on your Laravel route
                    type: 'GET',
                    headers: {
                        ...
                        render: function(data, type, full, meta) {
                            if (full.driver) {
                                var name = full.driver.user.name,
                                    ...
                                    baseUrl = route('driver.edit', full.driver_id);
                                var row_output =
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            return (
                                ...
                                '<a href="' + route('trip.edit', data.id) +
...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">{{ _t('Drivers') }}</h5>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                <div class="col-md-4 user_role"></div>
                <div class="col-md-4 user_plan"></div>
                <div class="col-md-4 user_status"></div>
                <div class="col-md-4">
                    <a href="{{ route('trip.create') }}" class="btn btn-primary mb-3"
                        id="newUserButton">{{ _t('New Trip') }}</a>
                </div>
            </div>
        </div>
